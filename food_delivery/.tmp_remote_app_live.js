...
// âï¸ NGROK URL (har safar yangilang)
...
async function apiRequest(path, options = {}) {
	try {
		...
		if (error instanceof TypeError) {
			showToast('â Internet yoâq yoki backend ishlamayapti');
		} else {
			showToast(`â ${error.message}`);
		}
		throw error;
	}
}
...
async function bootstrap() {
	if (!tg.initData) {
		showToast('â Telegram ichida oching');
		return;
	}
